---
name: episteme-health
description: Run deterministic structural health checks for an Episteme vault when explicitly invoked with `Episteme: health` or `/episteme health`. Regenerate graph-first index artifacts, classify findings by validation tier, and propose corrective diffs without applying them.
---
...
- Unresolved/broken wikilinks (reported as structural debt; apply/commit blocking is scoped to introduced links or links in modified notes)
...
**Note on apply/commit scope:** Existing unresolved links in untouched notes are Tier 1 debt only. Apply/commit blocking is governed by the apply/commit guardrails (newly introduced unresolved links and unresolved links in modified notes).
