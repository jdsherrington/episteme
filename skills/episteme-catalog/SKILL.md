---
name: episteme-catalog
description: Deterministically regenerate Episteme `_GRAPH.tsv` and `_CATALOG.tsv` artifacts when invoked by `Episteme: catalog` or `/episteme catalog`, or when called internally by Episteme setup/health/apply workflows. Enforce canonical parsing, graph-first generation, and byte-stable TSV output.
---
...
- Stable + volatile overdue `review_due`
